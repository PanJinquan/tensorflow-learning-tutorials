...
"""
    ...
    @File   : tf_record_demo.py
    @Author : panjq
    @E-mail : pan_jinquan@163.com
    @Date   : 2018-07-27 17:19:54
    @desc   : 将图片数据保存为单个record文件
"""
...
def disp_records(record_file,resize_height, resize_width,show_nums=4):
    ...
    '''
    # 读取record函数
    tf_image, tf_height, tf_width, tf_depth, tf_label = read_records(record_file,
                                                                     resize_height,
                                                                     resize_width,
                                                                     type='normalization')
    # 显示前4个图片
...
def batch_test(record_file,resize_height, resize_width):
    ...
    image_batch, label_batch = tf.train.batch([tf_image, tf_label],
                                                      batch_size=batch_size,
                                                      capacity=capacity)
...
if __name__ == '__main__':
    ...
    output_record_dir = 'dataset/record/train.tfrecords'
    resize_height = 224  # 指定存储图片高度
    resize_width = 224  # 指定存储图片宽度
    shuffle=True
    log=200
    # 产生record文件
    create_records(image_dir,train_file, output_record_dir, resize_height, resize_width,shuffle,log)
    example_nums=get_example_nums(output_record_dir)
    print("save example nums={}".format(example_nums))
    # 测试显示函数
