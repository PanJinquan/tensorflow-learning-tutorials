...
def find_exists(images_list,labels_list,sizeTh=1000):
    ''' 去除不存的文件和文件过小的文件列表
    :param images_list:
    :param labels_list:
    ...
    while i<len(images_list):
        path=images_list[i]
        if not ((os.path.exists(path)) and (os.path.getsize(path)>sizeTh)):
            print("non-existent or empty file:{}".format(path))
            images_list.pop(i)
            labels_list.pop(i)
            i -= 1
        i += 1
...
def read_images(filename,images_dir,reshape,shuffle=False,type=None):
    ...
    images_list, labels_list=find_exists(images_list, labels_list)
...
if __name__=="__main__":
    images_dir="../dataset/dataset_regression/test_image"
    filename="../dataset/dataset_regression/test_image.txt"
