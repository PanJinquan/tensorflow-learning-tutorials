...
def net_evaluation(sess,loss,accuracy,val_images_batch,val_labels_batch,val_nums):
    ...
    for _ in xrange(val_max_steps):
        ...
        val_loss,val_acc = sess.run([loss,accuracy],feed_dict={x: val_x, y: val_y, is_training: False})
...
def train(train_record_file,
    ...
    output = googlenet_v1.googlenet(x, labels_nums, is_training=is_training, verbose=True)


    # 定义训练的loss函数。
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=output, labels=y))
    train_step = tf.train.AdamOptimizer(learning_rate=base_lr).minimize(loss)
    ...
    with tf.Session() as sess:
        ...
        for i in range(max_steps+1):
            ...
            _, train_loss = sess.run([train_step, loss], feed_dict={x:batch_x, y:batch_y, is_training:True})
            # train测试(这里仅测试训练集的一个batch)
            if i%train_log_step == 0:
                train_acc = sess.run(accuracy,feed_dict={x:batch_x, y: batch_y, is_training: False})
