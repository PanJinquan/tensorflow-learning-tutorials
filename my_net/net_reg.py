...
def net_reg(inputs,labels_nums,reg,keep_prob):
...
def train(train_record_file,
    ...
    combine=net_reg(inputs=input_images,labels_nums=labels_nums,reg=reg,keep_prob=keep_prob)
    ...
    # loss = tf.reduce_sum(tf.losses.softmax_cross_entropy(onehot_labels=input_labels, logits=combine))
    ...
    # accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(combine, 1), tf.argmax(input_labels, 1)), tf.float32))
